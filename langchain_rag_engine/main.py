...
# --- Local Imports ---
from .rag.query_engine import stream_legal_assistant, query_legal_assistant
...
SECRET_KEY = os.environ.get("SECRET_KEY", "super-secret-key-that-is-not-safe-for-prod")
...
async def chat_stream(signed: str, db: Session = Depends(get_db)):
    try:
        ...
        return StreamingResponse(
            asyncio.as_completed([json.dumps({"type": "error", "message": e.detail})]),
    ...
    if not request_context:
        ...
        return StreamingResponse(
             asyncio.as_completed([json.dumps({"type": "error", "message": "Invalid request or already processed."})]),
             media_type="text/event-stream"
        )
