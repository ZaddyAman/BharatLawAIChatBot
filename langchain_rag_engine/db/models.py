...
class StreamSession(Base):
    """Database-backed storage for streaming sessions to replace in-memory registries"""
...
class TaskRegistry(Base):
    """Database-backed task registry for scalability"""
    __tablename__ = "task_registry"

    id = Column(Integer, primary_key=True, index=True)
    task_id = Column(String, unique=True, index=True)
    request_id = Column(String, index=True)
    task_type = Column(String)  # streaming, processing, etc.
    status = Column(String, default="running")  # running, completed, failed
    task_metadata = Column(JSON, nullable=True)  # Store additional task data
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now(), default=func.now())
