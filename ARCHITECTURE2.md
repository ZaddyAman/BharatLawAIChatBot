...
```mermaid
graph TB
    classDef clientLayer fill:#3B82F6,stroke:#1E40AF,stroke-width:2px,color:#FFFFFF
    classDef apiLayer fill:#10B981,stroke:#047857,stroke-width:2px,color:#FFFFFF
    classDef businessLayer fill:#8B5CF6,stroke:#7C3AED,stroke-width:2px,color:#FFFFFF
    classDef dataLayer fill:#F59E0B,stroke:#D97706,stroke-width:2px,color:#000000
    classDef infraLayer fill:#6B7280,stroke:#4B5563,stroke-width:2px,color:#FFFFFF

    subgraph "Client Layer"
        A[React Frontend]:::clientLayer
        B[Vite Dev Server]:::clientLayer
        C[Browser Client]:::clientLayer
    end

    subgraph "API Gateway Layer"
        D[FastAPI Application]:::apiLayer
        E[CORS Middleware]:::apiLayer
        F[JWT Authentication]:::apiLayer
        G[Rate Limiting]:::apiLayer
    end

    subgraph "Business Logic Layer"
        H[RAG Engine]:::businessLayer
        I[Auth Service]:::businessLayer
        J[Streaming Service]:::businessLayer
        K[Legal Analysis Engine]:::businessLayer
    end

    subgraph "Data Layer"
        L[(PostgreSQL)]:::dataLayer
        M[(Pinecone Vector DB)]:::dataLayer
        N[Voyage AI Embeddings]:::dataLayer
        O[OpenRouter LLM]:::dataLayer
    end

    subgraph "Infrastructure Layer"
        P[Docker Containers]:::infraLayer
        Q[Railway Platform]:::infraLayer
        R[Background Tasks]:::infraLayer
        S[Monitoring & Logging]:::infraLayer
    end
...
```mermaid
graph TD
    classDef mainApp fill:#3B82F6,stroke:#1E40AF,stroke-width:2px,color:#FFFFFF
    classDef apiLayer fill:#10B981,stroke:#047857,stroke-width:2px,color:#FFFFFF
    classDef businessLogic fill:#8B5CF6,stroke:#7C3AED,stroke-width:2px,color:#FFFFFF
    classDef dataAccess fill:#F59E0B,stroke:#D97706,stroke-width:2px,color:#000000
    classDef services fill:#6B7280,stroke:#4B5563,stroke-width:2px,color:#FFFFFF

    subgraph "Main Application"
        A[main.py]:::mainApp
        B[config.py]:::mainApp
        C[dependency_container.py]:::mainApp
    end

    subgraph "API Layer"
        D[auth.py]:::apiLayer
        E[acts.py]:::apiLayer
        F[feedback.py]:::apiLayer
    end

    subgraph "Business Logic"
        G[advanced_rag.py]:::businessLogic
        H[hybrid_search.py]:::businessLogic
        I[intent_classifier.py]:::businessLogic
        J[cot_reasoning.py]:::businessLogic
    end

    subgraph "Data Access"
        K[database.py]:::dataAccess
        L[models.py]:::dataAccess
        M[crud.py]:::dataAccess
    end

    subgraph "Services"
        N[scalable_registry.py]:::services
        O[metadata_filter.py]:::services
        P[prompt_chains.py]:::services
    end
